...
drop trigger if exists update_agent_feedback_items_fts on public.agent_feedback_items;
create trigger update_agent_feedback_items_fts
    before insert or update on public.agent_feedback_items
    for each row execute procedure tsvector_update_trigger(fts, 'pg_catalog.english', title, body);
...
drop policy if exists "Agent feedback items readable by service role" on public.agent_feedback_items;
create policy "Agent feedback items readable by service role"
    on public.agent_feedback_items for select
    using (auth.role() = 'service_role');

drop policy if exists "Agent feedback items writable by service role" on public.agent_feedback_items;
create policy "Agent feedback items writable by service role"
    on public.agent_feedback_items for all
    using (auth.role() = 'service_role')
    with check (auth.role() = 'service_role');

drop policy if exists "Agent feedback votes writable by service role" on public.agent_feedback_votes;
create policy "Agent feedback votes writable by service role"
    on public.agent_feedback_votes for all
    using (auth.role() = 'service_role')
    with check (auth.role() = 'service_role');

drop policy if exists "Agent feedback links writable by service role" on public.agent_feedback_links;
create policy "Agent feedback links writable by service role"
    on public.agent_feedback_links for all
    using (auth.role() = 'service_role')
    with check (auth.role() = 'service_role');

drop policy if exists "Agent feedback events writable by service role" on public.agent_feedback_events;
create policy "Agent feedback events writable by service role"
