...
(function () {
  ...
  function parseConversation(rawText, platform, overrideFirstSpeaker) {
    ...
    if (selected === 'chatgpt') {
      ...
      if (chatgptResult) {
        return { ...chatgptResult, messages: collapseSameRole(chatgptResult.messages) };
      }
    }
    ...
    return { ...genericResult, messages: collapseSameRole(genericResult.messages) };
  }

  function collapseSameRole(messages = []) {
    const out = [];
